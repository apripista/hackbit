...
(iii) Login Process
    ...
    The system checks if the user is verified and whether tfa is enabled.
    For tfa-enabled accounts, a six-digit token is generated and sent to the user's email for verification.
...
(iv) User Profile Management
    ...
    Enable tfa: Users can activate or deactivate tfa by providing the correct email and command.
...
- `tfa` (character varying(1)): Indicates if Two-Factor Authentication (tfa) is enabled (T) or disabled (F).
- `auth_token` (character varying(6)): Authentication token for various purposes like during account logging in and deletion of tfa enabled accounts.
